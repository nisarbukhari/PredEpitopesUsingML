...
evaluator = function(subset) {
  ...
  results = sapply(1:k, function(i) {
    ...
    tree  = nnet(as.simple.formula(subset, "cluster"), train, size=10, linout=TRUE, skip=TRUE, MaxNWts=10000, trace=FALSE, maxit=100)
...
eval = function(attr.subset) {
  ...
  results = sapply(1:k, function(i) {
    ...
    tree = nnet(formula, train, size=10, linout=TRUE, skip=TRUE, MaxNWts=10000, trace=FALSE, maxit=100)
    error.rate = sum(test$cluster != round(predict(tree, test),0)) / nrow(test)
