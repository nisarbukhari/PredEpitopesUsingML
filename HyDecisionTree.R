...
modelName
InputDataFileName="Peptides.csv"
InputDataFileName
...
training=80      # define percentage

##Load the data from file

dataset <- read.csv(InputDataFileName)    # Read the datafile
head(dataset)   # Display Top 6 records
nrow(dataset)   # Display number of records
names(dataset)  # Display fields names or columns names
ncol(dataset)
...
head(trainSample)     # Display Top 6 indecies
length(trainSample)   # Display number of indecies
ncol(trainSample)

testSample <- setdiff(seq_len(nrow(dataset)), trainSample)
head(testSample)      # Display Top 6 indecies
length(testSample) 
ncol(testSample)

####Feature Weighting (3 techniques, uncomment to run)
...
evaluator = function(subset) {
  ...
  results = sapply(1:k, function(i) {
    ...
    tree  = rpart(as.simple.formula(subset, "cluster"), train)
...
####Optimal Feature subset finding (3 techniques, uncomment to run)
...
head(trainDataset)    # Display Top 6 records
nrow(trainDataset)
...
Pred <- (predict(model, testDataset, type = 'vector' ))
...
eval = function(attr.subset) {
  ...
  results = sapply(1:k, function(i) {
    ...
    tree = rpart(formula, train, method="class",parms=list(split="information"), control=rpart.control(usesurrogate=0, maxsurrogate=0))
    #error.rate = sum(test$cluster != round(predict(tree, test),0)) / nrow(test)
